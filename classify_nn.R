...
logit.1 = glm(IS_SYS_ERROR  ~ base.2 +base.1 + base + diff_err_diff_dir + diff_error_dir + t_test, family=binomial(link=logit))
...
if ((length(hetero_locations[,1])+length(sys_errs[,1]))!= length(na.omit(bind_pred)[,1])){
	stop("ERROR in lengths \n");	
}

write.table(sys_errs,file=outf_sys,quote=F,row.names=F);
write.table(hetero_locations,file= outf_hetero,quote=F,row.names=F);
